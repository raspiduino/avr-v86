...
// Emulator vars
unsigned char *regs8, tmpvar1, tmpvar2, *opcode_stream, xlat_opcode_id, raw_opcode_id, extra, i_reg4bit, i_w, i_d, seg_override_en, rep_override_en, i_reg, i_mod, i_mod_size, i_rm, seg_override, scratch_uchar, rep_mode, io_ports[IO_PORT_COUNT], io_hi_lo, spkr_en, int8_asap, trap_flag;
unsigned short *regs16, reg_ip, ip;
...
#define MUL_MACRO(op_data_type,out_regs) (set_opcode(0x10), \
                                          out_regs[i_w + 1] = (op_result = (op_data_type)readmem(rm_addr) * (op_data_type)*out_regs) >> 16, \
                                          regs16[REG_AX] = op_result, \
                                          set_OF(set_CF(op_result - (op_data_type)op_result)))
#define DIV_MACRO(out_data_type,in_data_type,out_regs) (scratch_int = (out_data_type)readmem(rm_addr)) && !(scratch2_uint = (in_data_type)(scratch_uint = (out_regs[i_w+1] << 16) + regs16[REG_AX]) / scratch_int, scratch2_uint - (out_data_type)scratch2_uint) ? out_regs[i_w+1] = scratch_uint - scratch_int * (*out_regs = scratch2_uint) : pc_interrupt(0)
...
{
    regs8[FLAG_CF] = !!new_CF;
}

// Set auxiliary flag
char set_AF(int new_AF)
{
    regs8[FLAG_AF] = !!new_AF;
}

// Set overflow flag
char set_OF(int new_OF)
{
    regs8[FLAG_OF] = !!new_OF;
}
...
{
    return 16 * regs16[reg_seg] + (unsigned short)(regs16[reg_ofs] + op);
}
...
void v86()
{
    ...
    regs16 = (unsigned short *)regs8; // Map regs8 to regs16
    ...
    while(true)
    {
        ...
        ip = regs16[REG_CS] + reg_ip; // Instruction pointer
        ...
        switch (xlat_opcode_id)
        {
            OPCODE_CHAIN 0: // Conditional jump (JAE, JNAE, etc.)
                ...
                reg_ip += (char)i_data0 * (i_w ^ ((unsigned char)regs8[bios_table_lookup(TABLE_COND_JUMP_DECODE_A, scratch_uchar)] || (unsigned char)regs8[bios_table_lookup(TABLE_COND_JUMP_DECODE_B, scratch_uchar)] || (unsigned char)regs8[bios_table_lookup(TABLE_COND_JUMP_DECODE_C, scratch_uchar)] ^ (unsigned char)regs16[bios_table_lookup(TABLE_COND_JUMP_DECODE_D, scratch_uchar)]));
            ...
            OPCODE_CHAIN 5: // INC|DEC|JMP|CALL|PUSH
                if (i_reg < 2)// INC|DEC
                    ...
                    regs16[REG_CS] = readmem(op_from_addr + 2), // JMP|CALL (far)
            ...
            OPCODE 12: // ROL|ROR|RCL|RCR|SHL|SHR|???|SAR reg/mem, 1/CL/imm (80186)
                ...
                switch (i_reg)
                {
                    ...
                    OPCODE 2: // RCL
                        tmpvar1 = readmem(rm_addr);
                        R_M_OP(tmpvar1, += (regs8[FLAG_CF] << (scratch_uint - 1)) + , scratch2_uint >> (1 + TOP_BIT - scratch_uint));
                        writemem(rm_addr, tmpvar1);
                        set_OF(SIGN_OF(op_result) ^ set_CF(scratch2_uint & 1 << (TOP_BIT - scratch_uint)))
                    OPCODE 3: // RCR
                        tmpvar1 = readmem(rm_addr);
                        R_M_OP(tmpvar1, += (regs8[FLAG_CF] << (TOP_BIT - scratch_uint)) + , scratch2_uint << (1 + TOP_BIT - scratch_uint));
                        writemem(rm_addr, tmpvar1);
            ...
            OPCODE 22: // OUT DX/imm8, AL/AX
                ...
                (scratch_uint == 0x40 || scratch_uint == 0x42) && (io_ports[0x43] & 6) && (writemem(0x469 + scratch_uint - (io_hi_lo ^= 1), regs8[REG_AL])); // PIT rate programming
                //scratch_uint == 0x43 && (io_hi_lo = 0, regs8[REG_AL] >> 6 == 2) && (SDL_PauseAudio((regs8[REG_AL] & 0xF7) != 0xB6), 0); // Speaker enable
                scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 6) && (writemem(0x4AD + !(io_ports[0x3D4] & 1), regs8[REG_AL])); // CRT video RAM start offset
                scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 7) && (scratch2_uint = ((readmem(0x49E)*80 + readmem(0x49D) + readmem(0x4AD)) & (io_ports[0x3D4] & 1 ? 0xFF00 : 0xFF)) + (regs8[REG_AL] << (io_ports[0x3D4] & 1 ? 0 : 8)) - (short)readmem(0x4AD), writemem(0x49D, scratch2_uint % 80), writemem(0x49E, scratch2_uint / 80)); // CRT cursor position
            ...
            OPCODE 44: // XLAT
                regs8[REG_AL] = readmem(segreg(seg_override_en ? seg_override : REG_DS, REG_BX, regs8[REG_AL]))
            OPCODE 45: // CMC
                regs8[FLAG_CF] ^= 1
            ...
            OPCODE 48: // Emulator-specific 0F xx opcodes
                switch ((char)i_data0)
                {
                    ...
                    OPCODE_CHAIN 3: // DISK_WRITE + READ :)))
                        ...
                        regs8[REG_AL] = seekandcheck(regs8[REG_DL], (unsigned)regs16[REG_BP] << 9)
                            ? disk((char)i_data0 == 3, regs8[REG_DL], segreg(REG_ES, REG_BX, 0), regs16[REG_AX])
                            : 0;
                }
        }
        ...
        // help us here.
        reg_ip += (i_mod*(i_mod != 3) + 2UL*(!i_mod && i_rm == 6))*i_mod_size + bios_table_lookup(TABLE_BASE_INST_SIZE, raw_opcode_id) + bios_table_lookup(TABLE_I_W_SIZE, raw_opcode_id)*(i_w + 1);
...
int disk(bool op, int disknum, unsigned short offset, unsigned short b)
{
    ...
    if(op)
    {
        ...
        for(i = 0; i <= b;){
            ...
            file.seek(((unsigned)regs16[REG_BP] << 9) + i);
